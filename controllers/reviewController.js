...
exports.review_form_get = function (req, res, next) {
    Chef.find(function (err, result) {
        if (err) { return next(err); }
        res.render('review_form', { title: 'Escribe tu reseña', chef_list: result});
    })
};
...
exports.review_form_post = [
    ...
    (req, res, next) => {
        ...
        if(!errors.isEmpty()){
            Chef.find(function (err, result) {
                if (err) { return next(err); }
                res.render('review_form', { title: 'Escribe tu reseña', chef_list: result, review: review, errors: errors.array()});
            });
            return;
        }
        else {
